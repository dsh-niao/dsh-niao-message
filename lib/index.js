...
/** 运行时默认配置（供 Config schema 默认值与运行期兜底共用）。 */
const DEFAULTS = {
  /** 总开关：false 时全部组静默。 */
  enabled: true,
...
/** 单个通知组的 schema 工厂（默认值取自 DEFAULTS）。 */
const groupSchema = (def) => Schema.object({
  enabled: Schema.boolean().default(def.enabled),
  sound: Schema.string().default(def.sound),
  title: Schema.string().default(def.title),
  message: Schema.string().default(def.message),
})

/**
 * 插件配置 schema（官方插件开发约定的标准形式）。
 * Cordis 加载本插件时用它校验配置并填充未提供字段的默认值；
 * 未知字段透传保留（不影响旧版 patch config 兼容）。
 */
export const Config = Schema.object({
  enabled: Schema.boolean().default(DEFAULTS.enabled),
  tool: Schema.string().default(DEFAULTS.tool),
  click: Schema.union([
    Schema.string(),
    Schema.object({
      open: Schema.string(),
      activate: Schema.string(),
    }),
  ]).default(DEFAULTS.click),
  allowRepeat: Schema.boolean().default(DEFAULTS.allowRepeat),
  throttleMs: Schema.number().default(DEFAULTS.throttleMs),
  approvalGraceMs: Schema.number().default(DEFAULTS.approvalGraceMs),
  pendingMaxAgeMs: Schema.number().default(DEFAULTS.pendingMaxAgeMs),
  pendingFile: Schema.string().default(DEFAULTS.pendingFile),
  configFile: Schema.string().default(DEFAULTS.configFile),
  groups: Schema.object({
    abnormal: groupSchema(DEFAULTS.groups.abnormal),
    waiting: groupSchema(DEFAULTS.groups.waiting),
    success: groupSchema(DEFAULTS.groups.success),
  }),
})
...
/** 运行期默认值兜底：合并 DEFAULTS 与调用方传入的配置（浅层 + 三组）。
 *  cordis 加载时 config 已由 Config schema 填充默认值，此处仅兜底
 *  直接调用（测试 / 非 cordis 场景），保证字段齐全。 */
function withDefaults(config = {}) {
  const g = config.groups || {}
  return {
    enabled: config.enabled ?? DEFAULTS.enabled,
    tool: config.tool ?? DEFAULTS.tool,
    click: config.click ?? DEFAULTS.click,
    allowRepeat: config.allowRepeat ?? DEFAULTS.allowRepeat,
    throttleMs: config.throttleMs ?? DEFAULTS.throttleMs,
    approvalGraceMs: config.approvalGraceMs ?? DEFAULTS.approvalGraceMs,
    pendingMaxAgeMs: config.pendingMaxAgeMs ?? DEFAULTS.pendingMaxAgeMs,
    pendingFile: config.pendingFile ?? DEFAULTS.pendingFile,
    configFile: config.configFile ?? DEFAULTS.configFile,
    groups: {
      abnormal: { ...DEFAULTS.groups.abnormal, ...(g.abnormal || {}) },
      waiting: { ...DEFAULTS.groups.waiting, ...(g.waiting || {}) },
      success: { ...DEFAULTS.groups.success, ...(g.success || {}) },
    },
  }
}
...
/**
 * 解析最终配置：DEFAULTS/schema 默认值 ← patch config ← 配置文件（面板保存，最高优先级）。
 */
...
async function handle(ctx, cfg, throttleMap, req, res) {
  ...
  try {
    switch (action) {
      ...
      case 'dismiss-all': {
        // 关键：-remove 若用 execFile（stdio: pipe 收集输出），进程会挂起
        // 不退出且通知不被移除（实测 5s 超时被杀）；改用 spawn + stdio:
        // 'ignore' 后 70–130ms 正常退出并成功移除。另加 3s 兜底强杀。
        const remove = spawn(cfg.tool, ['-remove', NOTIFICATION_GROUP], { stdio: 'ignore' })
        const killer = setTimeout(() => remove.kill(), 3000)
        remove.on('exit', () => clearTimeout(killer))
        remove.on('error', (error) => ctx.logger?.warn(`dsh-niao-message: remove notifications failed: ${error.message}`))
